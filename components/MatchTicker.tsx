...
export default function MatchTicker({ matches, onMatchClick }: MatchTickerProps) {
  return (
    <div className="bg-slate-950 border-b border-slate-800 py-3 overflow-x-auto no-scrollbar">
      <div className="container mx-auto px-4 flex space-x-4 space-x-reverse min-w-max items-center">
        ...
        {matches.length === 0 ? (
          <div className="text-slate-500 text-xs px-4">لا توجد مباريات جارية اليوم في الدوريات المختارة</div>
        ) : (
          matches.map((match) => (
            <button
              key={match.id}
