  ...
  return (
    <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden shadow-xl">
        ...
        {TABS.map((tab) => (
          <button
            ...
            className={`flex-1 py-2 text-xs font-bold whitespace-nowrap px-3 transition-colors ${
              activeTab === tab.key ? 'text-white bg-slate-800 border-b-2 border-primary' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'
            }`}
          >
            {tab.label}
          </button>
            ...
            <tbody className="divide-y divide-slate-800">
              {filtered.map((team) => (
                <tr key={team.team} className="hover:bg-slate-800/30 transition-colors group">
                  <td className={`py-2 px-2 text-center font-bold ${team.rank <= 4 ? 'text-primary' : 'text-slate-500'}`}>{team.rank}</td>
      ...
      <div className="p-2 text-center bg-slate-800/50 border-t border-slate-800">
        <a href="#" className="text-[10px] text-primary font-bold block w-full hover:text-white transition-colors">
          جدول الترتيب الكامل
        </a>
      </div>
    </div>
