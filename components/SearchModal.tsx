    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-20 px-4">
      ...
      <div className="relative w-full max-w-2xl bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="overflow-y-auto p-2">
          {query && results.length === 0 && <div className="text-center py-10 text-slate-500">لا توجد نتائج مطابقة لـ &quot;{query}&quot;</div>}
            ...
            <div className="p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {['الهلال', 'النصر', 'رونالدو', 'الدوري السعودي', 'الكويت'].map((tag) => (
                  <button key={tag} onClick={() => setQuery(tag)} className="px-3 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-full text-sm transition-colors">
                    {tag}
                  </button>
                ))}
              </div>
          ...
          {results.map((article) => (
            <Link key={article.id} href={`/article/${article.id}`} onClick={onClose} className="flex items-start gap-4 p-3 hover:bg-slate-800/50 rounded-xl transition-colors group">
        ...
        {results.length > 0 && <div className="bg-slate-950 p-2 text-center border-t border-slate-800 text-[10px] text-slate-500">عرض {results.length} نتائج</div>}
      </div>
    </div>
  );
}
