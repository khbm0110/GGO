...
export default function NewsCard({ article, featured = false, compact = false }: NewsCardProps) {
  ...
  return (
    <Link
      ...
      className={`group relative block overflow-hidden rounded-xl bg-slate-900 border border-slate-800 hover:border-slate-700 transition-all duration-300 flex flex-col ${
    ...
    >
      ...
      <div className={`p-5 flex-1 flex flex-col justify-between ${featured ? 'absolute bottom-0 left-0 right-0 z-20' : ''}`}>
        <div>
          <h3
            className={`font-bold text-white leading-tight mb-2 group-hover:text-primary transition-colors ${
          ...
          {featured && (
            <p className="text-slate-300 text-sm md:text-base line-clamp-2 mb-4 hidden md:block">{article.summary}</p>
          )}
        </div>
