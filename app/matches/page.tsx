            ...
            return (
              <div key={league} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-lg">
                <div className="bg-gradient-to-r from-slate-800 to-slate-900 px-6 py-4 border-b border-slate-700 flex justify-between items-center">
                  ...
                  <span className="text-xs font-bold text-slate-400 px-3 py-1 bg-slate-800 rounded-full border border-slate-700">
                ...
                <div className="divide-y divide-slate-800/50">
                  {leagueMatches.map((match) => (
                    <div
                      key={match.id}
                      className="p-5 flex flex-col md:flex-row items-center justify-between hover:bg-slate-800/30 transition-colors gap-4 cursor-pointer group"
                    >
                      ...
                      <div className="flex flex-col items-center justify-center w-full md:w-32 order-2 bg-slate-900 py-2 rounded-lg border border-slate-800 group-hover:border-slate-700 transition-colors">
                        ...
                        <span className={`text-[10px] font-bold mt-1 ${match.status === 'LIVE' ? 'text-red-500 animate-pulse' : 'text-slate-500'}`}>
