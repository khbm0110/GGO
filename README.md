...
1. **ميزات متبقية**: مقارنة اللاعبين/الفرق، الخرائط الحرارية وxG، الترجمة متعددة اللغات، الوضع الفاتح
2. **ربط Appwrite** (آخر خطوة): إنشاء `appwrite-provider.ts` و`appwrite-auth.ts`، تفعيل الـ middleware
   للمصادقة الحقيقية، وتجهيز الـ API endpoints بحيث تصلح مباشرة لتطبيق Android/Kotlin

## المُنجز حتى الآن

الصفحات: الرئيسية، المباريات + توقع النتائج، الترتيب الكامل، الأندية، صفحة نادٍ (تشكيلة FIFA)،
صفحة لاعب، صفحة مدرب، الفيديو، صفحات الدول/البطولات، تسجيل الدخول/حساب، الملف الشخصي (Dream Team)،
ترتيب المتوقعين (Leaderboard)، لوحة إدارة كاملة (مقالات، أندية، مستخدمون، تعليقات، رعاة، SEO، إعدادات).

بالإضافة: بحث فوري، تعليقات متداخلة، إشعارات (تجريبية)، PWA قابل للتثبيت.
